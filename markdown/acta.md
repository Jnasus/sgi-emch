...
| 3. Cumplir con los plazos establecidos para el desarrollo, pruebas y despliegue del sistema. | | No exceder los plazos del cronograma de sprints. El sistema debe estar en producción el 24/08/2026. | |
...
| El proyecto consiste en el desarrollo e implementación de un sistema web interno para la gestión integral del inventario de equipos informáticos y el registro de incidentes técnicos en la EMCH "CFB". El sistema integrará los siguientes módulos:  • Módulo de Usuarios y Seguridad: autenticación local con 5 roles (Administrador, Jefe DTIC, Subjefe DTIC, Técnico de Campo, Directivos).  • Módulo de Inventario: registro y gestión de equipos (PCs y laptops), control de estados, histórico de bajas y carga masiva de áreas.  • Módulo de Incidentes: tickets auto-numerados, SLAs por tipo (Hardware, Software, Red, Impresora) y generación automática de PDF al cierre.  • Módulo de Reportes y Dashboard: paneles diferenciados por perfil, reportes por área, antigüedad e incidentes, exportación a Excel y PDF.  • Módulo de Notificaciones: alertas automáticas por email y mensaje interno cuando el stock de equipos caiga por debajo del umbral configurable (20%).  El desarrollo se realizará bajo metodología ágil Scrum, en 5 sprints, con despliegue en el servidor institucional de la EMCH "CFB". El proyecto se ejecutará entre el 05/04/2026 y el 24/08/2026. | | | |
...
| 1. La EMCH "CFB" debe asignar un líder usuario (TCO2 EP MORALES PEREZ) para coordinar el levantamiento de información.  2. La EMCH debe proveer acceso al servidor institucional para pruebas y despliegue.  3. Se debe entregar el inventario actual en Excel para la migración inicial de datos.  4. La institución debe asignar disponibilidad del personal DTIC para validaciones. | | 1. El sistema debe contar con registro y gestión de equipos informáticos (PCs y laptops).  2. El sistema debe gestionar 5 roles de usuario con control de acceso por módulo.  3. El sistema debe registrar incidentes técnicos con tickets, SLAs y generación de PDF.  4. El sistema debe generar reportes exportables a Excel y PDF.  5. El sistema debe enviar alertas automáticas de stock mínimo por email y notificación interna. | |
| **Restricciones** | | | |
| 1. El proyecto debe realizarse dentro de los plazos establecidos en el cronograma (05/04/2026 – 24/08/2026).  2. El sistema se desplegará exclusivamente en el servidor institucional de la EMCH "CFB" (uso interno).  3. En la fase actual no se contempla integración con Active Directory ni portal de reporte para usuarios finales.  4. Cada integrante del equipo debe cumplir sus entregables en los plazos de cada sprint. | | | |
...
| Fase 0 – Kick-off, arquitectura, Product Backlog y entorno listo | | Todos | 14/04/2026 |
| Sprint 1 – Módulo Usuarios y Seguridad (autenticación, roles, CRUD) | | Chavarría / Andia / Orozco | 04/05/2026 |
| Sprint 2 – Módulo Inventario (CRUD, estados, histórico, carga masiva) | | Chavarría / Orozco / Pariona / Andia | 25/05/2026 |
| Sprint 3 – Módulo Incidentes (tickets, SLAs, PDF automático) | | Chavarría / Orozco / Pariona / Andia | 14/06/2026 |
| Sprint 4 – Módulo Reportes, Dashboard y exportación Excel/PDF | | Chavarría / Orozco / Pariona / Andia | 17/07/2026 |
| Sprint 5 – Notificaciones, integración, pruebas finales y corrección de bugs | | Todos | 11/08/2026 |
| Fase Cierre – Pruebas de aceptación, despliegue, documentación y entrega final | | Todos | 24/08/2026 |
